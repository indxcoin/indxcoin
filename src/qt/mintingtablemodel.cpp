...
QVariant MintingTableModel::data(const QModelIndex &index, int role) const
{
    ...
    switch(role)
    {
      ...
      case Qt::BackgroundColorRole:
        int minAge = (IsProtocolV01(GetAdjustedTime()) ? params.nStakeMinAgeV01 : params.nStakeMinAge) / 60 / 60 / 24;
        int maxAge = (IsProtocolV01(GetAdjustedTime()) ? params.nStakeMaxAgeV01 : params.nStakeMaxAge) / 60 / 60 / 24;
...
{
    //const CBlockIndex *pblock = GetLastBlockIndex(::ChainActive().Tip(), true);
    double difficulty = 1; //GetDifficulty(pblock);  
