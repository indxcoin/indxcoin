...
These tools must be installed in order to build Indxcoin Core from source.
...
### 4. Clone Indxcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Indxcoin Core repository to a directory.
All build scripts and commands will run from this directory.

``` bash
git clone https://github.com/indxcoin/indxcoin.git
```
...
It is not necessary to build wallet functionality to run `indxcoind` or  `indxcoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/indxcoin/indxcoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Indxcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/indxcoin/indxcoin/issues/7714).
...
You can deploy a `.dmg` containing the Indxcoin Core application using `make deploy`.
...
## Building Indxcoin Core

### 1. Configuration

There are many ways to configure Indxcoin Core, here are a few common examples:
...
Run the following in your terminal to compile Indxcoin Core:
...
## Running Indxcoin Core

Indxcoin Core should now be available at `./src/indxcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/indxcoin-qt`.

The first time you run `indxcoind` or `indxcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Indxcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Indxcoin"

touch "/Users/${USER}/Library/Application Support/Indxcoin/indxcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Indxcoin/indxcoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Indxcoin/debug.log
```

## Other commands:

```shell
./src/indxcoind -daemon      # Starts the indxcoin daemon.
./src/indxcoin-cli --help    # Outputs a list of command-line options.
./src/indxcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/indxcoin-qt -server # Starts the indxcoin-qt server mode, allows indxcoin-cli control
```
