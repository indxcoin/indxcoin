...
def initialize_datadir(dirname, n, chain):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "indxcoin.conf"), n=n, chain=chain)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "indxcoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "indxcoin.conf")):
        with open(os.path.join(datadir, "indxcoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
