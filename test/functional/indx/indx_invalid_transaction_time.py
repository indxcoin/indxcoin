...
class InvalidTransactionTime(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # create valid transaction and mine block with it
        ...
        block = self.mine_pos_block(extra_txs=[tx])
        self.send_blocks([block])

        # create invalid transaction and add it to node mempool
        invalid_tx = create_tx_with_script(
            tx, n=0, amount=500000 * 100000000 - 200000, script_pub_key=CScript([self.coinbase_pubkey, OP_CHECKSIG]))
        invalid_tx.nVersion = 2
        invalid_tx.nTime = 0xFFFFFFFF
        self.sign_tx(invalid_tx, tx)

        tx_hash = node.sendrawtransaction(invalid_tx.serialize().hex())
        print("Node accepted our invalid_tx", tx_hash)
        print("Mempool:", node.getmempoolinfo())
        print(node.getrawtransaction(tx_hash, True))

        # we can't mine new PoS block using node (missing implementation)
        # so we'll just generate block invalid here and try to add it
        # it will be rejected because it contains invalid transaction
        block = self.mine_pos_block(extra_txs=[invalid_tx])
        self.send_blocks([block], success=False, reject_reason='bad-tx-time', reconnect=True)
        print("Block with invalid tx was rejected")
        mempool = node.getmempoolinfo()
        print("Mempool:", mempool)
        assert mempool['size'] == 1
        print("Invalid transaction stayed in mempool")
