...
class CoinbaseInflation(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create PoS block with additional reward in coinbase
        coinbase = self.create_pos_coinbase(
            self.block_heights[self.tip.sha256] + 1)
        coinbase_output = CTxOut()
        coinbase_output.nValue = 100000000 * 100000000  # a lot of extra coins
        coinbase_output.scriptPubKey = CScript([OP_TRUE])
        coinbase.vout.append(coinbase_output)
        coinbase.rehash()

        block = self.mine_pos_block(coinbase=coinbase)
        self.send_blocks([block])

        # print coinbase
        tx = node.getrawtransaction(coinbase.hash, True)
        print("Coinbase transaction")
        pprint(tx)

        # mine 100 more pos blocks to mature coinbase
        blocks = []
        for i in range(100):
            blocks.append(self.mine_pos_block())
        self.send_blocks(blocks)

        # make sure coins from coinbase can be spend
        coinbase_transfer = create_tx_with_script(
            coinbase, n=1, amount=coinbase_output.nValue, script_pub_key=self.coinbase_pubkey)
        self.sign_tx(coinbase_transfer, coinbase, out_n=1)

        block = self.mine_pos_block(extra_txs=[coinbase_transfer])
        self.send_blocks([block])

        # mine 5 more pos blocks to get confirmation
        blocks = []
        for i in range(5):
            blocks.append(self.mine_pos_block())
        self.send_blocks(blocks)
        ...
        # get transaction
        tx = node.getrawtransaction(coinbase_transfer.hash, True)
        print("Transfer of coinbase transaciton")
        pprint(tx)
