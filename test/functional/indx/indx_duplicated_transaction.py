...
class DuplicatedTransaction(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create PoS block with custom coinbase
        coinbase = self.create_pos_coinbase(1000)

        block = self.mine_pos_block(coinbase=coinbase)
        self.send_blocks([block])

        # print transaction
        tx1 = node.getrawtransaction(coinbase.hash, True)
        print("Original transaction")
        pprint(tx1)

        # create another block with the same coinbase as previous blocks
        block = self.mine_pos_block(coinbase=coinbase)
        self.send_blocks([block])

        # print transaction again
        tx2 = node.getrawtransaction(coinbase.hash, True)
        print("Duplicated transaction")
        pprint(tx2)

        assert tx1['hash'] == tx2['hash']
        assert tx1['blockhash'] != tx2['blockhash']
        print("Transaction was duplicated (check blockhash field)")
