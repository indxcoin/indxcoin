...
class IncorrectStakeModifierCache(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        amount = 500000 * 100000000 - 100000
        output = self.get_spendable_output()
        tx = create_tx_with_script(
            output, n=0, amount=amount, script_pub_key=CScript([self.coinbase_pubkey, OP_CHECKSIG]))
        tx.nVersion = 2
        tx.nTime = block_time - 7 * 24 * 60 * 60
        self.sign_tx(tx, output)

        # mine block with new transaction
        valid_block = self.mine_pos_block(extra_txs=[tx])
        self.send_blocks([valid_block])
        # mine one more valid block
        valid_block = self.mine_pos_block(time_adjustment=10)
        self.send_blocks([valid_block])

        invalid_block = self.mine_pos_block(time_adjustment = 8 * 60 * 60, stake=tx)
        # try to add block with previous transaction as a stake, it should fail
        print("Adding invalid block, it should fail")
        self.send_blocks([invalid_block], success=False, reconnect=True)
        print("success, it failed")

        # lets set tip to last valid block and add 2 more blocks
        print("Adding two valid blocks")
        self.tip = valid_block
        # you can comment this two blocks, then later invalid blocks will be rejected
        self.send_blocks([self.mine_pos_block(time_adjustment = 8 * 60 * 60)])
        self.send_blocks([self.mine_pos_block(time_adjustment = 1)])

        # change tip to 3 blocks ago
        self.tip = valid_block

        # lets try to add invalid block again
        print("Trying to add another invalid block")
        invalid_block = self.mine_pos_block(time_adjustment = 8 * 60 * 60 + 1, stake=tx)

        # add few more blocks on top of invalid block
        invalid_blocks = [invalid_block]
        for i in range(5):
            invalid_blocks.append(self.mine_pos_block(time_adjustment = 1))

        self.send_blocks(invalid_blocks)
        print("Invalid block was accepted")
    ...
    def create_pos_coinstake(self, stake, block_time, transfer_value=10):
        tx = CTransaction()
        tx.nVersion = 2
        tx.nTime = block_time
        tx.vin.append(CTxIn(COutPoint(stake.sha256, 0)))
    ...
    def mine_pos_block(self, stake=None, coinbase=None, coinstake=None, extra_txs=None, time_adjustment=24 * 60 * 60):
        block_time = self.tip.nTime + time_adjustment
...
if __name__ == '__main__':
    IncorrectStakeModifierCache().main()
